...
class Pix2PixHDModel(BaseModel):
    ...
    def forward(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose, grid, mask_fore, external_cloth, external_cloth_mask):
        # Encode Inputs
        ...
        fake_c, warped_original, warped_mask_original, warped_grid= self.Unet(clothes, fake_cl_dis, pre_clothes_mask,grid)

        ## Replace with externals.
        warped = external_cloth
        warped_mask = external_cloth_mask
    ...
    def forward1(self, label, pre_clothes_mask, img_fore, clothes_mask, clothes, all_clothes_label, real_image, pose,grid,mask_fore):
        # Encode Inputs
        input_label, masked_label, all_clothes_label = self.encode_input(label, clothes_mask, all_clothes_label)
        arm1_mask = torch.FloatTensor((label.cpu().numpy() == 11).astype(np.float)).cuda()
        arm2_mask = torch.FloatTensor((label.cpu().numpy() == 13).astype(np.float)).cuda()
    ...
    def inference(self, label, label_ref, image_ref):
        ...
        # Encode Inputs
        print('debug yichuc start inference.')
